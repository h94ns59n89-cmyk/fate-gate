...
export function buildPersonalityTagsPrompt(baziData: Record<string, unknown>, analysis: { wang_shuai: string; yong_shen: string; ji_shen: string }): string {
  return `# 角色
...
# 预分析结论（由专业八字引擎计算）
- 旺衰: ${analysis.wang_shuai}
- 用神: ${analysis.yong_shen}
- 忌神: ${analysis.ji_shen}

# 标签写作要求
1. 标签中的日主五行必须与输入日主一致（不可写错）
2. 核心人格标签和副标签要基于十神和旺衰得出，结合性格特征而非命理术语
3. 综合参考用神/忌神方向，判断性格中的优势和需要注意的方面
...
export function buildFullReportPrompt(baziData: Record<string, unknown>, analysis: { wang_shuai: string; yong_shen: string; ji_shen: string; reasoning: string }): string {
  return `# 角色
...
- 日主: ${baziData.dayMaster ?? '[数据不可用]'}
- 四柱: ${fmt(baziData.pillars)}
- 五行旺衰(得分=地支藏干计数，偏旺=3，旺=4): ${fmt(baziData.fiveElements)}
- 十神分布: ${fmt(baziData.shishen)}
- 大运: ${fmt(baziData.dayun)}

# 预分析结论（由专业八字引擎计算，不可修改，直接用在报告里）
- 旺衰: ${analysis.wang_shuai}
- 用神: ${analysis.yong_shen}
- 忌神: ${analysis.ji_shen}
- 推理过程: ${analysis.reasoning}

你必须严格按照上述结论写报告。personality 章节的 wang_shuai/yong_shen/ji_shen 必须与预分析结论一致。
所有章节的"建议"和"分析"必须关联用神/忌神五行方向，但用日常语言表达。
...
{
  "overall": <整数数字，0-100>,
  "career": { "score": <整数数字，0-100>, "label": "<四字以内的运势评级，如'稳中有进''机会增多'>", "text": "<针对事业的详细建议，必须关联用神/忌神，30-50字>" },
  "wealth": { "score": <整数数字，0-100>, "label": "<四字以内评级>", "text": "<针对财运的详细建议，关联五行生克，30-50字>" },
  "relationships": { "score": <整数数字，0-100>, "label": "<四字以内评级>", "text": "<针对感情人际的详细建议，关联十神配置，30-50字>" },
  "health": { "score": <整数数字，0-100>, "label": "<四字以内评级>", "text": "<针对健康的详细建议，关联五行旺衰，30-50字>" },
  "advice": "<年度整体建议，必须关联用神/忌神，30-60字>",
