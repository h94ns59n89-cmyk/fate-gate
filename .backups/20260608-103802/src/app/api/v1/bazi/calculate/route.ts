...
export const POST = withMiddleware(async (req) => {
  ...
  const location = resolveBirthLocation({
    ...
    timezone: typeof timezone === 'number' ? timezone : 8,
  });

  if (!location) {
    return error(100103, '出生地点暂未识别，请填写城市名，如：上海市', 400);
  }
  ...
  if (user_id) {
    try {
      ...
      console.warn('[Bazi] DB unavailable or persistence failed, returning transient result:', (err as Error)?.message);
    }
  }
