...
describe('Bazi Calculator', () => {
  ...
  it('should resolve detailed address through geocoding fallback', async () => {
    ...
    const location = await resolveBirthLocation({ birthPlace: '上海市黄浦区南京东路100号' });

    expect(fetchMock).toHaveBeenCalledTimes(2);
    expect(location?.place).toBe('上海');
    expect(location?.longitude).toBeCloseTo(121.4752, 4);
    expect(location?.latitude).toBeCloseTo(31.2347, 4);
    expect(location?.timezone).toBe(8);
